...
public:
	...
	void traceChunk(std::vector<std::shared_ptr<Pixel>> pixels, int start, int stride, const std::shared_ptr<Scene>& scene);

	//calculates the color of the surfaces
	glm::vec3 surfaceColor(std::shared_ptr<Scene> scene, glm::vec3 intersection, const std::shared_ptr<Surface>& surface,
